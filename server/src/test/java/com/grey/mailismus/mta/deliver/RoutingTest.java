...
public class RoutingTest
{
	...
	public void testDestinationRoutes() throws java.io.IOException
	{
		...
		org.junit.Assert.assertFalse(rly.isAuthRequired());
		org.junit.Assert.assertNull(rly.getUsername());
		...
		org.junit.Assert.assertTrue(rly.isAuthRequired());
		org.junit.Assert.assertEquals("user1", rly.getUsername());
		org.junit.Assert.assertEquals("pass1", rly.getPassword().toString());
		org.junit.Assert.assertNull(rly.getSslConfig());

		rt = parseRouting(cfgxml_interceptor, false, true, true);
		verifyDestinationRoute(rt, "anydomain", false, "10.0.0.1", Protocol.TCP_PORT);
		verifyDestinationRoute(rt, "destdomain1", true, "10.0.0.2", Protocol.TCP_PORT);
		rly = rt.getInterceptor();
		org.junit.Assert.assertTrue(rly.dnsOnly);
		org.junit.Assert.assertEquals(IP.IP_LOCALHOST, rly.getAddress().ip);
		org.junit.Assert.assertEquals(2001, rly.getAddress().port);
		org.junit.Assert.assertNull(rly.senders);

		rt = parseRouting(cfgxml_interceptor_withslave, true, true, true);
		verifyDestinationRoute(rt, "anydomain", false, "10.0.0.1", Protocol.TCP_PORT);
		rly = rt.getInterceptor();
		org.junit.Assert.assertFalse(rly.dnsOnly);
		org.junit.Assert.assertEquals(IP.IP_LOCALHOST, rly.getAddress().ip);
		org.junit.Assert.assertEquals(Protocol.TCP_PORT, rly.getAddress().port);
		org.junit.Assert.assertNotNull(rly.getSslConfig());
		org.junit.Assert.assertFalse(rly.getSslConfig().isLatent());
		org.junit.Assert.assertFalse(rly.getSslConfig().isMandatory());
		org.junit.Assert.assertNull(rly.senders);
	}
	...
	public void testSourceRoutes() throws java.io.IOException
	{
		...
		org.junit.Assert.assertEquals(IP.convertDottedIP("10.0.0.2"), rly.getAddress().ip);
		...
		rly = rt.getSourceRoute(emaddr, 0);
		org.junit.Assert.assertEquals(IP.convertDottedIP("10.0.0.3"), rly.getAddress().ip);
		...
		rly = rt.getSourceRoute(emaddr, 0);
		org.junit.Assert.assertEquals(IP.convertDottedIP("10.0.0.4"), rly.getAddress().ip);
	...
	public void testSourceRoutesWithIP() throws java.io.IOException
	{
		...
		Relay rly = rt.getSourceRoute(emaddr, 0);
		org.junit.Assert.assertEquals(IP.convertDottedIP("10.0.0.5"), rly.getAddress().ip);
	...
	private void verifyDestinationRoute(Routing rt, String destdom, boolean exp_rt, String exp_ip, int exp_port)
	{
		...
		if (exp_ip == null) {
			...
			org.junit.Assert.assertEquals(IP.convertDottedIP(exp_ip), rly.getAddress().ip);
			org.junit.Assert.assertEquals(exp_port, rly.getAddress().port);
			org.junit.Assert.assertNull(rly.senders);
		}
	}
}